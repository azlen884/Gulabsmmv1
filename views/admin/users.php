        ...
        <div>
          ...
          <div class="flex items-center justify-between mb-3">
            <div class="flex items-center gap-3">
              <div class="w-10 h-10 rounded-2xl bg-rose-50 text-rose-600 font-bold flex items-center justify-center text-sm">
              ...
              <div>
                <h3 class="font-bold text-sm text-slate-800 flex items-center gap-2">
                  ...
                  <?php if ($u['role'] === 'admin'): ?>
                    <span class="px-2 py-0.5 rounded-full bg-slate-900 text-white text-[10px] font-bold">ADMIN</span>
                  <?php endif; ?>
                </h3>
                <div class="text-[11px] text-slate-400">@<?= e($u['username']) ?> • <?= e($u['email']) ?></div>
              </div>
            </div>
            ...
            <form method="POST">
              ...
              <button type="submit" class="px-2.5 py-1 rounded-full text-xs font-bold transition-colors <?= $u['status'] === 'active' ? 'bg-emerald-50 text-emerald-600 hover:bg-emerald-100' : 'bg-red-50 text-red-600 hover:bg-red-100' ?>">
                <?= ucfirst($u['status']) ?>
              </button>
            </form>
          </div>
          ...
          <div class="grid grid-cols-3 gap-2 py-3 border-y border-slate-100 mb-4 text-xs">
            <div>
              ...
              <span class="font-extrabold text-slate-900 text-sm">$<?= number_format($u['balance'], 2) ?></span>
            </div>
            <div>
              <span class="text-slate-400 block text-[10px]">Total Spent:</span>
              <span class="font-bold text-slate-700 text-sm">$<?= number_format($u['total_spent'], 2) ?></span>
            </div>
            <div>
              <span class="text-slate-400 block text-[10px]">Orders:</span>
              <span class="font-bold text-slate-700 text-sm"><?= number_format($u['total_orders']) ?></span>
            </div>
          </div>
        </div>

        <!-- Quick Balance Adjust Form -->
        <form method="POST" class="flex items-center gap-2 pt-2 border-t border-slate-100 text-xs">
          <input type="hidden" name="action" value="adjust_balance">
          <input type="hidden" name="user_id" value="<?= $u['id'] ?>">
          <select name="type" class="px-2 py-1.5 rounded-lg border border-slate-200 bg-slate-50 text-[11px] font-bold text-slate-700">
          ...
          >
          <button type="submit" class="px-3 py-1.5 rounded-lg bg-rose-500 hover:bg-rose-600 text-white font-bold text-[11px] transition-colors">
            Apply
          </button>
        </form>
      </div>
